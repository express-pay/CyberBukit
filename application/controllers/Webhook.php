...
class Webhook extends CI_Controller {
	...
	public function expresspay() {  //expresspay
		if ($_SERVER['REQUEST_METHOD'] === 'POST') 
		{
			$payloadArray['data'] = (isset($_POST['Data'])) ? htmlspecialchars_decode($_POST['Data']) : '';
			$payloadArray['signature'] = (isset($_POST['Signature'])) ? $_POST['Signature'] : '';
			$this->load->library('m_payment');
			$paymentVerifyArray = $this->m_payment->verifyPayment('expresspay', $payloadArray);

			if ($paymentVerifyArray['success']) {
				$query = $this->db->where('gateway_identifier', $paymentVerifyArray['gateway_identifier'])->where('callback_status', 'pending')->get('payment_log', 1);
				if ($query->num_rows()) {  //pay for the first time, no matter one-time or recurring
					$rs = $query->row();
					$chargedArray['identifier'] = $paymentVerifyArray['gateway_identifier'];
					$chargedArray['eventID'] = $paymentVerifyArray['gateway_identifier'];
					$this->handlePayment($chargedArray, $rs);
					if ($rs->type == 'subscription') { $this->handleNewSubscription($chargedArray, $rs); }
				}
				else {  // maybe it's an update of recurring?
					$query_log = $this->db->where('gateway_event_id', $paymentVerifyArray['gateway_identifier'])->get('payment_log', 1);
					if ($query_log->num_rows()) {  //prevent scams of duplication
						$subscriptionArray = $this->m_payment->retrieveSubscription('expresspay', $paymentVerifyArray['gateway_identifier']);
						if ($subscriptionArray['success']) {
							$chargedArray['identifier'] = $paymentVerifyArray['gateway_identifier'];
							$chargedArray['amount'] = $paymentVerifyArray['data']->Amount;
							$this->handleUpdatedSubscription($subscriptionArray, $chargedArray);
						}
					}
				}
		...
		else
		{
			...
			exit('FAILED | request method not supported');
		}
	}
}